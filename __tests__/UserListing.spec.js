...
describe('Listing Users', () => {
  ...
  it('returns 6 users in page content when there are active 6 users and inactive 5 users in the database', async () => {
    await addUsers(6, 5);
    const response = await getUsers();
  ...
  it('returns 10 users and corresponding size indicator when size is set 0', async () => {
    await addUsers(11);
  ...
  it('returns user page without logged in user when request has valid authorization', async () => {
    ...
    //normally we should have 2 pages with 11 users
    //but since we exclude 1 logged in user it should be 1 page
...
describe('Get User', () => {
  ...
  it('returns not found message when user not found', async () => {
    const response = await getUser();
